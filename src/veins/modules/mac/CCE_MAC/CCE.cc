...
#include <math.h> /* exp */
#include <veins/modules/mac/CCE_MAC/CCE.h>
...
void CCE::handleSelfMsg(cMessage *msg)
{
 ...
 else if (msg == nextMacEvent)
 {
		...
		//TODO inject the CW into the packet here...
		pktToSend->setCwPck(ContentionWindow);
...
void CCE::handleUpperControl(cMessage *msg)
{
 assert(false);
}

double CCE::reward_function(int cw_level)
{
 counter = 0;
 std::sort(vCW.begin(), vCW.end());
 //when cw FOUND IN VECTOR
 for (int i = 0; i < vCW.size(); i++)
 {
		if (vCW[i] == cw[cw_level])
		{
			counter++;
		}
 }
 cw_frequency = float(counter) / float(vCW.size());

 //We know CW level and corresponding frequency. Update table?
 //maps are sorted according to the key
 //add to table
 CWfrequencies.at(cw_level) = cw_frequency;

 sortedfrequencies = CWfrequencies;
 if (sortedfrequencies.size() == 7)
 {
		...
		for (i = 0; i < 7; i++)
		{
			if (CWfrequencies[cw_level] == sortedfrequencies[i])
				index = i; //return reward[index]
		}
 }
 return reward_CCE[index];
}

void CCE::handleUpperMsg(cMessage *msg)
{
 ...
 if (par("learn").longValue() == 1)
 {
		...
		/*
																 ...
																 */
		if ((lifetime < trainTime) && (epsilon > 0.05))
		{
 ...
 */
		...
		if (thisMsg->getAcknowledged() < 2)
		{
			...
			{ //ACK RECEIVED
				...
				{ //currentAction increase 1, decrease -1

					//reset negative reward

					//JUST CCE//
					reward = reward_function(currentState);
					//reward=reward_function(currentState)*reward_delay[currentState];
					//reward=(0.5*reward_function(currentState))+(0.5*reward_delay[currentState]);//give positive reward (binary)

					//reward=1;
					//HISTOGRAM REWARD
					//reward = reward_freq[ContentionWindow];
				}
			...
			else if (thisMsg->getAcknowledged() == 0)
			{ //NO ACK RECEIVED
			...
			//else{ //if it is an original message coming from app layer, ack=2, let me know of my app type ATM.
			// }

			preCumulativeReward = preCumulativeReward + Q[currentState][currentAction];
			//record cumReward over time
			/*
 ...
 //this is an explorative=1/exploitative=0 message? we need only exploitative to be taken into account when calculating CW frequencies
 thisMsg->setExplore(explore);
 ...
 myfile << myEDCA[chan]->myQueues[ac].cwMin << "," << myRateType << "," << epsilon << "," << reward << "," << cw_best << "," << window << "," << simTime() << std::endl;
 myfile.close();
 /*
...
void CCE::handleLowerControl(cMessage *msg)
{
...
void CCE::setActiveChannel(t_channel state)
{
 activeChannel = state;
 assert(state == type_CCH || (useSCH && state == type_SCH));
}

void CCE::csvWrite(std::string filename, int A)
{
 std::ofstream myfile;
...
int CCE::maximum(int state, bool returnIndexOnly)
{
...
void CCE::finish()
{
 ...
 csvWrite(directory + "rate.csv", myRateType);

 //csvWrite(directory+"CW2.csv",ContentionWindow);
}

void CCE::attachSignal(Mac80211Pkt *mac, simtime_t startTime, double frequency, uint64_t datarate, double txPower_mW)
{
...
Signal *CCE::createSignal(simtime_t start, simtime_t length, double power, uint64_t bitrate, double frequency)
{
...
simtime_t CCE::timeLeftTillGuardOver() const
{
 ASSERT(useSCH);
...
simtime_t CCE::timeLeftInSlot() const
{
 ASSERT(useSCH);
...
void CCE::changeServiceChannel(int cN)
{
 ASSERT(useSCH);
...
void CCE::setTxPower(double txPower_mW)
{
 txPower = txPower_mW;
}
void CCE::setMCS(enum PHY_MCS mcs)
{
...
void CCE::setCCAThreshold(double ccaThreshold_dBm)
{
 phy11p->setCCAThreshold(ccaThreshold_dBm);
}

void CCE::handleLowerMsg(cMessage *msg)
{
 ...
 {
		//if you receive a packet from radio
		statsReceivedBroadcasts++;

		if (simTime().dbl() < (recTime + 1.0) && (wsm->getExplore() == 0) && (wsm->getRateType() == myRateType))
		{ //acks awell
			packetspersec++;
		}
		else if (simTime().dbl() > (recTime + 1.0) && (wsm->getExplore() == 0) && (wsm->getRateType() == myRateType))
		{
			recTime = simTime().dbl();

			if (packetspersec > window)
			{
				window = packetspersec;
			}
			packetspersec = 0;
		}

		//adaptive window for CCE, changes every second according to num of useful packets received/s

		//fetch CW value from packet - save into vector and process histogram
		//only direct messages(no retransmissions), only exploitative messages considered.

		//over that window
		CWnew = wsm->getCwPck();
		if (vCW.size() < window)
		{
			if ((wsm->getExplore() == 0) && (wsm->getAcknowledged() == 2) && (wsm->getRateType() == myRateType))
			{
				vCW.push_back(CWnew);
			}
			else
			...
			//reset frequency measurements every 100 samples
			/*
      if (myRateType==1){
          refresh=0.001;
      }
      else if(myRateType==2){
          refresh=0.004;
      }
 */
			refresh = 0.001;

			if (uniform(0, 1) > refresh)
			{
				reward_max = 0;
			}
			if (reward_max < reward_function(currentState))
			{
				reward_max = reward_function(currentState);
				cw_best = cw[currentState];
			}
		}
		sendUp(wsm);
 }
...
int CCE::EDCA::queuePacket(t_access_category ac, WaveShortMessage *msg)
{
...
int CCE::EDCA::createQueue(int aifsn, int cwMin, int cwMax, t_access_category ac)
{
...
CCE::t_access_category CCE::mapPriority(int prio)
{
...
WaveShortMessage *CCE::EDCA::initiateTransmit(simtime_t lastIdle)
{
...
simtime_t CCE::EDCA::startContent(simtime_t idleSince, bool guardActive)
{
...
void CCE::EDCA::stopContent(bool allowBackoff, bool generateTxOp)
{
 //update all Queues
...
void CCE::EDCA::backoff(t_access_category ac)
{
...
void CCE::EDCA::postTransmit(t_access_category ac)
{
...
void CCE::channelBusySelf(bool generateTxOp)
{
...
void CCE::channelIdle(bool afterSwitch)
{
...
void CCE::setParametersForBitrate(uint64_t bitrate)
{
...
simtime_t CCE::getFrameDuration(int payloadLengthBits, enum PHY_MCS mcs) const
{
 simtime_t duration;
 if (mcs == MCS_DEFAULT)
 {
