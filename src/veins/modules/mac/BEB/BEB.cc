...
void BEB::handleSelfMsg(cMessage* msg) {
...
void BEB::handleUpperControl(cMessage* msg) {
	assert(false);
}

void BEB::handleUpperMsg(cMessage* msg) {
	...
	lifetime = simTime().dbl() - startTime;
	 ...
	 //Pseudo BEB with Application Layer ACKs
	if(thisMsg->getAcknowledged()<2){
	    if(thisMsg->getAcknowledged()==1){ //ACK OK!
	        //TODO CW=CWmin;
	        cw_level=0;
	        delete thisMsg;
	    }
	    else if(thisMsg->getAcknowledged()==0){ //NO ACK
	        if (ContentionWindow<200){ //IF NOT MAX
	            cw_level++;
	            //TODO CW=CW*2 ARRAY +1 tofix
	            delete thisMsg;
	        }
	    }
	    ContentionWindow=cw[cw_level];
			return;
	}
	...
	//ALWAYS - no need to learn this time
	...
	if (num == 1 && idleChannel == false && myEDCA[chan]->myQueues[ac].currentBackoff == 0 && chan == activeChannel) {
	   ...
	   //RECORD CW variation over time
	    ...
	    myfile<<myEDCA[chan]->myQueues[ac].cwMin<<","<<epsilon<<","<<simTime()<<std::endl;
	    myfile.close();
	    /*
...
void BEB::handleLowerControl(cMessage* msg) {
...
void BEB::setActiveChannel(t_channel state) {
	activeChannel = state;
	assert(state == type_CCH || (useSCH && state == type_SCH));
}

void BEB::csvWrite(std::string filename,int A){
	std::ofstream myfile;
...
int BEB::maximum(int state, bool returnIndexOnly){
...
void BEB::attachSignal(Mac80211Pkt* mac, simtime_t startTime, double frequency, uint64_t datarate, double txPower_mW) {
...
Signal* BEB::createSignal(simtime_t start, simtime_t length, double power, uint64_t bitrate, double frequency) {
...
simtime_t BEB::timeLeftTillGuardOver() const {
	ASSERT(useSCH);
...
simtime_t BEB::timeLeftInSlot() const {
	ASSERT(useSCH);
...
void BEB::changeServiceChannel(int cN) {
	ASSERT(useSCH);
...
void BEB::setTxPower(double txPower_mW) {
	txPower = txPower_mW;
}
void BEB::setMCS(enum PHY_MCS mcs) {
	ASSERT2(mcs != MCS_DEFAULT, "invalid MCS selected");
	bitrate = getOfdmDatarate(mcs, BW_OFDM_10_MHZ);
	setParametersForBitrate(bitrate);
}

void BEB::setCCAThreshold(double ccaThreshold_dBm) {
	phy11p->setCCAThreshold(ccaThreshold_dBm);
}

void BEB::handleLowerMsg(cMessage* msg) {
...
int BEB::EDCA::queuePacket(t_access_category ac,WaveShortMessage* msg) {
...
int BEB::EDCA::createQueue(int aifsn, int cwMin, int cwMax,t_access_category ac) {
...
BEB::t_access_category BEB::mapPriority(int prio) {
...
WaveShortMessage* BEB::EDCA::initiateTransmit(simtime_t lastIdle) {
...
simtime_t BEB::EDCA::startContent(simtime_t idleSince,bool guardActive) {
...
void BEB::EDCA::stopContent(bool allowBackoff, bool generateTxOp) {
	//update all Queues
...
void BEB::EDCA::backoff(t_access_category ac) {
...
void BEB::EDCA::postTransmit(t_access_category ac) {
...
void BEB::channelBusySelf(bool generateTxOp) {
...
void BEB::channelIdle(bool afterSwitch) {
...
void BEB::setParametersForBitrate(uint64_t bitrate) {
...
simtime_t BEB::getFrameDuration(int payloadLengthBits, enum PHY_MCS mcs) const {
    simtime_t duration;
    if (mcs == MCS_DEFAULT) {
