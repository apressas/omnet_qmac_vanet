...
#include "veins/modules/application/MultihopApp/MultihopApp.h"

using Veins::TraCIMobilityAccess;
using Veins::AnnotationManagerAccess;

const simsignalwrap_t MultihopApp::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);

Define_Module(MultihopApp);

void MultihopApp::initialize(int stage) {
...
void MultihopApp::onBeacon(WaveShortMessage* wsm) { //when beacon is received from other nodes (->from lower layer)
...
void MultihopApp::onData(WaveShortMessage* wsm) {
...
void MultihopApp::sendMessage(std::string blockedRoadId) {
	sentMessage = true;
...
void MultihopApp::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details) {
	/*Enter_Method_Silent();
...
void MultihopApp::handleParkingUpdate(cObject* obj) {
...
void MultihopApp::handlePositionUpdate(cObject* obj) {
...
void MultihopApp::sendWSM(WaveShortMessage* wsm) {
